...
Both of these methods are registered with FastAPI and can be accessed through the EidOS API. 
...
Actions are used to create long-running processes that are be monitored and controlled by the AgentOS and are used to implement the EidOS conversation model.
...
This one method now implements the EidOS conversation model and can be used to create a conversational agent.
...
Resources are the generic way every object is defined in EidOS.
...
Any object that is defined in EidOS is a resource and can be referenced in this way. For example, you could define a LogicUnit resource and reference it in many agent definitions.
...
AgentPrograms, allowing them to be started, stopped, and managed with ease. AgentOS interfaces with FastAPI to enable automatic generation of endpoints, making the deployment of agents in the EidOS environment streamlined. With decorators like @register_program
