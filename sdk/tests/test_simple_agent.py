...
image_compatible_cpu = dict(llm_unit=dict(
...
resources = [
    ...
    r("json_output", actions=[
        dict(output_schema=dict(type="object",
                                properties=dict(capital=dict(type="string"), population=dict(type="number"))))
    ]),
    r("states", actions=[
    ...
    r("optional_file_no_body", actions=[dict(files="single-optional", user_prompt="How many legs does the animal have?")], cpu=image_compatible_cpu),
    r("single_file", actions=[dict(files="single")], cpu=image_compatible_cpu),
    r("single_file_no_body", actions=[dict(files="single", user_prompt="How many legs does the animal have?")], cpu=image_compatible_cpu),
    r("multiple_files", actions=[dict(files="multiple")], cpu=image_compatible_cpu),
    r("multiple_files_no_body", actions=[dict(files="multiple", user_prompt="what do these images have in common?")], cpu=image_compatible_cpu),
]
...
async def test_multiple_prompt_args():
    resp = await Agent.get('multiple_prompt_args').program("converse",
                                                           body=dict(a1="What is the capital of", a2="France?"))
    assert "paris" in resp.data.lower()
...
async def test_refs():
    resp = await Agent.get('refs').program("converse",
                                           body="Start a conversation with system_prompt and what its favorite country is.")
...
async def test_multiple_files(cat, dog):
    resp = await Agent.get('multiple_files').program(
        "converse",
        data=dict(body="what do these images have in common?"),
