...
from eidos.agent.agent import Agent, register_action, AgentState, AgentSpec, register_program, EidolonHandler
...
class GenericAgent(Agent, Specable[GenericAgentSpec]):
    ...
    async def question(self, process_id, **kwargs) -> AgentState[LlmResponse]:
        ...
        response = await t.schedule_request(
            prompts=[UserTextCPUMessage(prompt=(env.from_string(self.spec.user_prompt).render(**body))), *image_messages],
