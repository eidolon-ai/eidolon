...
from eidolon_sdk.vector_store.embeddings import OpenAIEmbeddingSpec, OpenAIEmbedding
from examples.code_search.code_sync import CodeSync
...
class CodeSearch(LogicUnit, Specable[CodeSearchConfig]):
    ...
    async def _init(self):
        if not self.syncer:
            self.syncer = CodeSync(os.path.abspath(self.root_dir), self.agent_memory)
