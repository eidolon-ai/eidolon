...
As of now, Eidolon has an ad hoc release process: releases are cut with high frequency by a developer and [published to pypi](https://pypi.org/project/eidolon-ai-sdk/).
...
* [openai](https://openai.com) as the default LLM. You will need an OPEN_AI key to run the project.
...
The core development is all done using python and if all you are interested in is the backend, you can ignore the typescript setup and run the UI via the docker-compose file.
...
There are 3 main components that need to be built and run: the client libraries, the server, and the examples:
