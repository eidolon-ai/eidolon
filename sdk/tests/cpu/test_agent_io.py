...
@pytest.fixture
def bus_event():
    return BusEvent(process_id="process1", thread_id=0, event_type="output_response", event_data={"response": "Test Response"})
...
class TestIOUnit:
    async def test_bus_read_output_response(self, io_unit, bus, bus_event, agent_cpu):
        ...
        await io_unit.bus_read(bus)
        agent_cpu.respond.assert_awaited_once_with(bus_event.process_id, bus_event.event_data["response"])
    ...
    def test_process_request_with_user_prompt(self, io_unit):
        ...
        io_unit.request_write.assert_called_once_with(BusEvent("process1", 0, "input_request", {"messages": [expected_message]}))
    ...
    def test_process_request_with_system_prompt(self, io_unit):
        ...
        io_unit.request_write.assert_called_once_with(BusEvent("process2", 0, "input_request", {"messages": [expected_message]}))
    ...
    def test_process_request_with_image_url_prompt(self, io_unit):
        ...
        expected_message = UserMessageImageURL(image_url="https://example.com/image.jpg")
        io_unit.request_write.assert_called_once_with(BusEvent("process3", 0, "input_request", {"messages": [UserMessage(content=[expected_message])]}))
    ...
    def test_process_request_with_mixed_prompts(self, io_unit):
        ...
        io_unit.request_write.assert_called_once_with(BusEvent("process4", 0, "input_request", {"messages": expected_messages}))
    ...
    def test_process_request_with_template_rendering(self, io_unit):
        ...
        expected_message = UserMessage(content=[UserMessageText(text="Hello, World")])
        io_unit.request_write.assert_called_once_with(BusEvent("process6", 0, "input_request", {"messages": [expected_message]}))
